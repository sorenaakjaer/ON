	...
	new Vue({
		...
		methods: {
			...
			fetchTicketDetails() {
				...
				fetch('https://dev-.dk/ppServices/api/dc/gettroubleticketdetails/' + encodeURIComponent(this.searchQuery), requestOptions)
					.then(response => {
						if (!response.ok) {
							throw new Error('Network response was not ok');
						}
			...
			fetchI18N() {
				...
				fetch("https://dev-.dk/ppServices/api/general/getDisplayDetails/gettroubleticketdetails?language=DA", requestOptions)
					.then(response => {
						if (!response.ok) {
							throw new Error('Network response was not ok');
						}
